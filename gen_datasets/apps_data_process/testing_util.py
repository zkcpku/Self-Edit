    ...
    if test is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            try:
                ...
                results.append(f"type 0 compilation error = {e} msg = {msg}")
                return results
            signal.alarm(0)
            ...
            try:
                ...
                # results.append(-2)
                msg = traceback.format_exc()
                results.append(f"type 1 compilation error = {e} msg = {msg}")
                return results
            signal.alarm(0)
        if debug:
        ...
        try:
            ...
            # results.append(-2)
            msg = traceback.format_exc()
            results.append(f"unable to get function error = {e} msg = {msg}")
            return results
        if debug:
            print(in_outs["inputs"])
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                try:
                    ...
                    # results.append(-1)
                    msg = traceback.format_exc()
                    results.append(f"Standard input runtime error or time limit exceeded error = {e} msg = {msg}")
                    continue
                faulthandler.disable()
                signal.alarm(0)
                if debug:
                ...
                with Capturing() as output:
                    try:
                        ...
                        # results.append(-1)
                        msg = traceback.format_exc()
                        results.append(f"Call-based runtime error or time limit exceeded error = {repr(e)}{e} msg = {msg}")
                    signal.alarm(0)
                ...
                # print(set(frozenset(s) for s in in_outs["outputs"][index]))
                try:
